		...
		<div class="container">
			...
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
				...
				<el-table-column label="操作" width="350" align="center">
					<template #default="scope">
						<el-button text :icon="Edit" @click="handleEditDescription(scope.$index, scope.row)" v-permiss="14">
							设置修缮规范
						</el-button>
						<el-button text :icon="Edit" @click="handleEditDate(scope.$index, scope.row)" v-permiss="14">
							更新最近修缮日期
						</el-button>
...
let idx1: number = -1;
const handleEditDescription = (index: number, row: any) => {
	idx1 = index;
	form.name = row.name;
	form.type = row.type;
	form.era = row.era;
...
let idx2: number = -1;
const handleEditDate = (index: number, row: any) => {
	idx2 = index;
	form.name = row.name;
	form.type = row.type;
	form.era = row.era;
